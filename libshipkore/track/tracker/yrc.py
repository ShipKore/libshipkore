...
class YRC(BaseTrackService):
    ...
    def _transform(self):
        ...
        data = {
            ...
            'estimated_date': parse(waybill_data.get('dueDate')) if waybill_data.get('dueDate') else None,
            'delivered_date' : parse(waybill_data.get('deliveryDate')) if waybill_data.get('deliveryDate') else None,
            'reference_no' : '',
            'destination' : waybill_data.get('destinationCity','')+' '+waybill_data.get('destinationState','')+' '+waybill_data.get('destinationZip',''),
            'receiver_name': ''
        }
        ...
        checkpoint = {
            ...
            "status": self.status_mapper(waybill_data.get('statusMessage')),
            "substatus": waybill_data.get('statusMessage'),
            "checkpoint_time": parse(waybill_data.get('statusDate')) if waybill_data.get('statusDate') else None,
            "state": None,
            "zip": None,
        }
