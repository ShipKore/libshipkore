...
class SpeeDee(BaseTrackService):
    ...
    def _transform(self):
        ...
        sub_status = selector.xpath('//*[@id="frameBody"]/table/tbody/tr[1]/td[2]/strong/text()').get()
        status = self.status_mapper(sub_status)
        delivered_date = None
        delivere_time= None
        if status == 'Delivered':
            delivered_date= selector.xpath('//*[@id="frameBody"]/table/tbody/tr[1]/td[4]/text()').get().split()[0]
            delivere_time= selector.xpath('//*[@id="frameBody"]/table/tbody/tr[1]/td[4]/text()').get().split()[1]
