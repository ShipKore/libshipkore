...
class SoutheasternFreightLines(BaseTrackService):
    ...
    def _transform(self):
        ...
        sub_status= selector.xpath('//*[@id="resultsTable"]/tbody/tr[13]/td[4]/text()').get().strip()
        status_content = selector.xpath("//td[contains(text(), 'Status:')]/following-sibling::td[1]/text()").getall()
        ...
        data = {
            ...
            'reference_no' : '',
            'destination' : selector.xpath('//*[@id="resultsTable"]/tbody/tr[10]/td[4]/a/text()').get(),
            'receiver_name': ''
        }
        checkpoints = []

        checkpoint_table = selector.xpath(f'//*[@id="detail{self.waybill}"]/table/tbody/tr')
