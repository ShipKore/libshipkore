...
class APCPostalLogistics(BaseTrackService):
    ...
    def _transform_checkpoint(self, scan):
        checkpoint = {
            ...
            "checkpoint_time": datetime.strptime( scan.get('date', ''), "%m.%d.%Y %I:%M:%S %p" ), #02.05.2022 01:27:00 PM
            "state": None,
            "zip": None
        }
    ...
    def _transform(self):
        ...
        data = {
            ...
            'status': self.status_mapper(waybill_data.get('events', [{}])[0].get('description')),
            'substatus': waybill_data.get('events', [{}])[0].get('description'),
            'estimated_date': datetime.strptime( waybill_data.get('estimatingDeliveryTime'), "%m.%d.%Y %I:%M:%S %p" ) if waybill_data.get('estimatingDeliveryTime') else None,
