...
class ShreeNandanCourier(BaseTrackService):
    ...
    def _transform(self):
        ...
        sub_status = selector.xpath('//*[@id="content_DeliveryStatusLabel"]//text()').get()
        date_time = selector.xpath('//*[@id="content_DeliveryDateLabel"]//text()').get().strip()
        print ('date_time')
        print (date_time)
        # date= None
        # time= None
        ...
        data = {
            ...
            'status': self.status_mapper(sub_status) if sub_status.strip() else StatusChoice.InTransit,
