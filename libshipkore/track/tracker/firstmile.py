...
class FirstMile(BaseTrackService):
    ...
    def _transform(self):
        ...
        sub_status = selector.xpath('//*[@id="packageDetail"]/div/article/section[1]/section/div/h2/text()').get()
        sub_status = sub_status.strip() if sub_status else ''
        status = self.status_mapper(sub_status)
        delivered_date = None
        if status == 'Delivered':
            delivered_date = selector.xpath('//*[@id="packageDetail"]/div/article/section[1]/section/div/h4/text()').get()
