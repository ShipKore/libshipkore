...
class GLS_USA(BaseTrackService):
    ...
    def _fetch(self):
        self.raw_data = requests.post(
            'https://www.gls-us.com/Tracking',
            data= {'TrackingNumber': self.waybill}
        ).text
        # print(self.raw_data, "###")

    def _transform(self):
        selector = parsel.Selector(text = self.raw_data)
        sub_status = selector.xpath('//*[@id="packagedetails"]/tr[4]/td[2]/span/text()').get()
        print(sub_status)
        ...
        data = {
            ...
            'estimated_date': None,
            'delivered_date' : parse(selector.xpath('//*[@id="packagedetails"]/tr[6]/td[2]/span/text()').get()),
            'delivered_time' : '',
            'reference_no' : '',
            'destination' : selector.xpath('//*[@id="packagedetails"]/tr[3]/td[2]/span/text()').get(),
            'receiver_name': ''
        }

        checkpoint_data = selector.xpath('//*[@class="trackingResults"]/div[@class="row result"]')
        checkpoints= []

        for rows in checkpoint_data:
            row = rows.xpath('.//div').getall()
            cols= []
